...
class ImageRepository {
  async createImage(
    ...
    try {
      ...
      const counter = await Image.countDocuments();
      ...
      if (images_arr.length > 0) {
        const createdImages = await Image.insertMany(images_arr); // Insert the accumulated images array
  ...
  async findById(id: string, image_key: string, next: NextFunction) {
    const result = await Image.findOne({ _id: id }).populate([
      { path: "audit_log", model: "User" },
      { path: "seo", model: "Karnal_web_seo" },
    ]);
    if (!result) {
  ...
  async update(data: any, image_key: string, next: NextFunction) {
    ...
    const isExist: any = await Image.findOne({
      _id: data?.id,
    }).populate("seo");
    if (!isExist) {
  ...
  async updateImage(
    ...
    try {
      ...
      if (oldImageId) {
        await Image.findByIdAndUpdate(oldImageId, updateData, {
          new: true,
        });
      }
